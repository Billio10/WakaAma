...
def assign_points_from_lif(lif_lines):
    """Extract club points from a .lif file if it is a Final race file.
       Handles multiple associations joined with '&'.
    """
    ...
    for line in lif_lines[1:]:  # skip header
        ...
        # Split associations joined with "&" and assign points to each
...
if not folder:
    ...
    # Aggregate points across all final races for the year
    for f in lif_files:
        ...
        # Optional: print file lines for verification
        for line in lif_lines:
            print(line)
        print("\n")  # Blank line between files

    # Display total scores for the year
    print("=== Total Scores Across All Final Races ===")
    ...
    messagebox.showinfo(
        "Total Scores",
        "\n".join(f"{club}: {points}" for club, points in total_scores.items())
    )
